...
public class BoardController {
    ...
    public String modify(BoardDto boardDto, Integer page, Integer pageSize, Model m, HttpSession session, RedirectAttributes rattr){
        ...
        try {
            int row_Cnt = boardService.modify(boardDto);
    ...
    public String write(BoardDto boardDto, Model m, HttpSession session, RedirectAttributes rattr){
        ...
        try {
            int row_Cnt = boardService.write(boardDto);
    ...
    public String write(Model m){
        ...
        return "board"; //읽기와 쓰기에 사용. 쓰기에 사용할 때엔, mode = new
    }

    @GetMapping("/read")
    public String read(String writer, Integer bno, Integer page, Integer pageSize, Model m, HttpSession session){
...
//            m.addAttribute("boardDto", boardDto); //아래 문장과 동일
            m.addAttribute(boardDto); //attributeName 생략 -> 밸류의 '타입'의 첫글자를 소문자로 한 것으로 설정됨
    ...
    public String list(SearchCondition sc, Model m, HttpServletRequest request) {
        ...
        try {
            ...
            //getPage를 호출 해서 list에 담고
            List<BoardDto> list = boardService.getSearchResultPage(sc);
            //해당 list를 모델 객체에 담음 -> 알아서 디스패처 서블릿
